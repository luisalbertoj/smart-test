...
export class ListComponent implements OnInit {
  ...
  actualizarDatos(): void {
    ...
    const ids: any = [];
    this.grupoSeleccionado.personas.forEach((element: any) => {
      ids.push(element.id || element);
    });
    this.grupoSeleccionado.personas = ids;
    this.factory
      .patch('grupo/' + this.grupoSeleccionado.id, this.grupoSeleccionado)
  ...
  seleccion(elemento: any, dato?: any): void {
  ...
  loadEstudiantes(): void {
    this.factory
      .getAll('rol?nombre=estudiante')
      .subscribe((response: any): any => {
        // console.log('RolEstudiante', response);
        if (!response.length) {
          return this.toast.error(
            'No existe el rol de estudiante en el sistema'
          );
        }
        this.factory
          .getAll('persona?idRol=' + response[0].id)
  ...
  selectEstudiantes(items: any): void {
