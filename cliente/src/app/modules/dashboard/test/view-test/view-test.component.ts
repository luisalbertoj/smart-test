...
export class ViewTestComponent implements OnInit {
  ...
  respuestaSeleccionada(index: number, respuesta: any): any {
  ...
  validarRespuestas(): any {
    ...
    for (let i = 0; i < this.respuestas.length; i++) {
      if (this.respuestas[i] === this.test.preguntas[i].respuestaCorrecta?.id) {
        ...
        if (this.test.preguntas[i].valor) {
          response.nota += parseInt(this.test.preguntas[i].valor);
        }
      }
    }
    let estadoTest = 1;
    ...
    this.factory
      .post('resultTestStudent', {
        ...
        estudiante: this.factory.user.id,
        preguntasTotales: response.totales,
        respuestasCorrectas: response.correctas.length,
        calificacionPreg: response.nota,
      })
      .subscribe(
        (res: any) => {
          console.log(res);
