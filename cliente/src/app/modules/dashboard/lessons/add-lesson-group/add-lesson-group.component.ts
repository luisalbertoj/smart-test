...
export class AddLessonGroupComponent implements OnInit {
  ...
  public loadLessons(): void {
    ...
    this.factory.getAll(`leccion?where={"titulo":{"contains":"${this.busqueda.trim()}"}}&select=id,titulo&populate=grupos`)
      .subscribe(arg => {
  ...
  public loadGrupo(): void {
    this.lecciones = [];
    this.factory.getAll(`grupo?where={"id":"${this.model.id}"}&select=id&populate=lecciones`)
      .subscribe(arg => {
  ...
  changeCheked(id: any): void {
    const i = this.lecciones.find((element, index) => {
      if(element === id) {
  ...
  actualizarModel() {
    this.factory.put('grupo', this.model.id, {lecciones: this.lecciones})
      .subscribe(arg => {
